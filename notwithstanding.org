...
#+begin_src emacs-lisp
(when (not (lookup-key (current-global-map) "C-x C-g"))
...
;; (global-set-key "C-'" 'j0ni/unicode-shortcut-map)
#+end_src
...
(straight-override-recipe '(synthwave-theme :type git :host github :local-repo "emacs-synthwave-theme" :repo "TroyFletcher/emacs-synthwave-theme"))
(straight-override-recipe '(carp-emacs :type git :host github :repo "carp-lang/carp-emacs"))
...
(straight-use-package 'carp-emacs)
...
(straight-use-package 'constant-theme)
...
(straight-use-package 'phoenix-dark-pink-theme)
...
(straight-use-package 'synthwave-theme)
...
This, like pixel scrolling, is something I didn't know I was missing. I tried it for a while and it turns out that I don't mind an extra keystroke in exchange for some flexibility. The emacs way.
...
I've used projectile for a long time, but it seems like project.el is actually entirely capable of supporting my use cases. So I've been using that mostly without extra configuration. However the questionnaire I have to peruse and fill out when I pick a project is annoying, and I would like to just go ahead and choose a file.

In the end though, projectile's ergonomics are just better.

#+begin_src emacs-lisp
(setq project-switch-commands 'projectile-switch-project)
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; (dolist (k '("C-j" "RET"))
  ;;   (define-key mct-minibuffer-completion-list-map (kbd k) #'mct-complete-and-exit)
  ;;   (define-key mct-minibuffer-local-completion-map (kbd k) #'mct-complete-and-exit)
  ;;   (define-key mct-minibuffer-local-filename-completion-map (kbd k) #'mct-complete-and-exit))
...
**** Corfu
Note: Corfu isn't switched on at the moment. It has potential to be much better than company-mode, but right now there are too many things that only kinda sorta work with corfu, and company still ends up being installed as a dependency anyway.

Why this and not company? Better integration, no special independent invocation (completion-at-point does not invoke company, so there are frequent collisions).
...
**** Company-mode
But somehow company-mode still ends up being hardcoded somewhere or other
...
(setq company-tooltip-idle-delay 0.7)
(setq company-global-modes '(not telega-chat-mode circe-channel-mode))
...
#+begin_src emacs-lisp
(add-hook 'after-init-hook 'nyan-mode) ;; nuff said
#+end_src
...
For a while I tried to get with the builtin project.el, but it just isn't enough.
...
This is a bit nasty though - once lsp-mode is loaded, it ignores lsp-keymap-prefix. So we have to do this, or go back to using a customization setting via use-package. I'm not even sure that would work. See https://github.com/emacs-lsp/lsp-mode/issues/1672

#+begin_src emacs-lisp
(setq lsp-keymap-prefix "C-c C-l")
;; now load the code, hopefully the mapping will happen
(require 'lsp-mode)
#+end_src
...
(add-hook 'lui-mode-hook
          (lambda ()
            ...
            (setq-local circe-color-nicks-everywhere t)
...
(setq modus-themes-hl-line '(neutral))
(setq modus-themes-completions nil)
(setq modus-themes-org-blocks 'tinted-background)
(setq modus-themes-headings nil) ;; check docs
(setq modus-themes-variable-pitch-ui nil)
(setq modus-themes-mode-line '(5 borderless accented)) ;; '(borderless accented)
...
#+end_src
**** Yeah...maybe Cyberpunk Neon?
#+begin_src emacs-lisp
(require 'cyberpunk-neon-theme)
;; (load-theme 'cyberpunk-neon t)
#+end_src
**** Phoenix Dark
#+begin_src emacs-lisp
...
(setq doom-ir-black-padded-modeline t)
...
(setq doom-manegarm-muted-modeline nil)
(setq doom-manegarm-padded-modeline nil)

(setq doom-miramare-brighter-comments nil)
(setq doom-miramare-padded-modeline nil)
...
**** Doom Modeline
Not strictly a theme, but this seems like the best place for it. Maybe this is really the eye candy section.
#+begin_src emacs-lisp
...
(add-hook 'css-mode-hook (lambda () (setq-local css-fontify-colors nil)))
#+end_src
...
;; fix comments to be compatible with VIM users
;; (add-hook 'clojure-mode-hook (lambda () (setq-local comment-column nil)))
#+end_src
...
(add-hook 'fennel-mode-hook (lambda () (setq-local inferior-lisp-program "fennel")))

;; for compiling lua to fennel
...
Org was installed and required before tangling this file.
...
;; hmm, also on the fence about this, let's try just laying it out
...
(add-hook 'message-mode 'org-msg-mode)
      ...
      (list (make-mu4e-context
            ...
            (make-mu4e-context
             ...
             :enter-func (lambda ()
                           (when (mu4e-running-p)
                             (mu4e-update-mail-and-index nil))
                           (mu4e-message "Switching to JTN context"))
             :match-func (lambda (msg)
                           (when msg
                             (string-match-p "^/JefferiesTube" (mu4e-message-field msg :maildir))))
             :vars `((user-mail-address . "joni@jefferiestube.net")
                     (user-full-name . "Jon Irving")
                     (mu4e-sent-messages-behavior . sent)
                     (mu4e-sent-folder . "/JefferiesTube/sent-mail")
                     (mu4e-trash-folder . "/JefferiesTube/trash")
                     (mu4e-drafts-folder . "/JefferiesTube/drafts")
                     (mu4e-refile-folder . "/JefferiesTube/all-mail")
                     (mu4e-maildir-shortcuts . (("/JefferiesTube/INBOX" . ?i)
                                                ("/JefferiesTube/sent-mail" . ?s)
                                                ("/JefferiesTube/drafts" . ?d)
                                                ("/JefferiesTube/trash" . ?t)))
                     (mu4e-bookmarks . ,(list (j0ni/mu4e-bookmark "JefferiesTube" "7" ?w)
                                              (j0ni/mu4e-bookmark "JefferiesTube" "30" ?m)))
                     (smtpmail-smtp-user . "joni@jefferiestube.net")
                     (smtpmail-smtp-server . "smtp.migadu.com")
                     (smtpmail-smtp-service . 465)
                     (smtpmail-stream-type . ssl)))
            (make-mu4e-context
             :name "Lollyshouse.net"
             :enter-func (lambda ()
                           (when (mu4e-running-p)
                             (mu4e-update-mail-and-index nil))
                           (mu4e-message "Switching to LHN context"))
             :match-func (lambda (msg)
                           (when msg
                             (string-match-p "^/Lollyshouse.net" (mu4e-message-field msg :maildir))))
             :vars `((user-mail-address . "j@lollyshouse.net")
                     (user-full-name . "Jon Irving")
                     (mu4e-sent-messages-behavior . sent)
                     (mu4e-sent-folder . "/Lollyshouse.net/sent-mail")
                     (mu4e-trash-folder . "/Lollyshouse.net/trash")
                     (mu4e-drafts-folder . "/Lollyshouse.net/drafts")
                     (mu4e-refile-folder . "/Lollyshouse.net/all-mail")
                     (mu4e-maildir-shortcuts . (("/Lollyshouse.net/INBOX" . ?i)
                                                ("/Lollyshouse.net/sent-mail" . ?s)
                                                ("/Lollyshouse.net/drafts" . ?d)
                                                ("/Lollyshouse.net/trash" . ?t)))
                     (mu4e-bookmarks . ,(list (j0ni/mu4e-bookmark "Lollyshouse.net" "7" ?w)
                                              (j0ni/mu4e-bookmark "Lollyshouse.net" "30" ?m)))
                     (smtpmail-smtp-user . "j@lollyshouse.net")
                     (smtpmail-smtp-server . "smtp.migadu.com")
                     (smtpmail-smtp-service . 465)
                     (smtpmail-stream-type . ssl)))
            (make-mu4e-context
